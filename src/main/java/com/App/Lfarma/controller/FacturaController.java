...
public class FacturaController {
    ...
    @PostMapping("/guardar")
    public String guardarFactura(@RequestParam String codigoCliente, @RequestParam List<Long> idsProductos, @RequestParam List<Integer> cantidades) {
        Cliente cliente = clienteService.obtenerClientePorCodigo(codigoCliente).orElseThrow(() -> new RuntimeException("Cliente no encontrado"));

        List<DetalleFactura> detalles = new ArrayList<>();
        for (int i = 0; i < idsProductos.size(); i++) {
            Producto producto = productoService.buscarPorId(idsProductos.get(i)).orElseThrow(() -> new RuntimeException("Producto no encontrado"));
    ...
    public String verDetalleFactura(@PathVariable Long id, Model model) {
        Factura factura = facturaService.obtenerFacturaPorId(id).orElseThrow(() -> new RuntimeException("Factura no encontrada"));
